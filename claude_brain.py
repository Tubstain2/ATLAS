"""
ATLAS Claude Brain — Module 1

Primary AI reasoning engine. Routes queries between:
  - Claude Sonnet  → complex reasoning, coding, research, self-improvement
  - ATLASCore      → control commands, simple voice replies (MLX/Groq)

Maintains full conversation history (last 20 turns) and saves sessions
...
- Your name is ATLAS. Never say you are Claude, an AI model, or any language model.
...
class ClaudeBrain:
    """
    Primary voice callback for ATLAS when Claude is available.

    Wire-up in main.py:
        brain = ClaudeBrain(config)
        brain.set_core(core)            # ATLASCore for control/web/edit/MLX
        brain.set_self_improve(engine)  # optional SelfImproveEngine
    ...
    def __init__(self, config: dict):
        ...
        self._model      = api_cfg.get("claude_model",  "claude-sonnet-4-6")
        ...
        self._lock        = Lock()
        self._routing_mode = "auto"      # 'auto' | 'claude' | 'core'

        self._core         = None        # ATLASCore
        self._self_improve = None        # SelfImproveEngine

        # conversations/ folder for session persistence
        ...
        if not key:
            log.warning("ANTHROPIC_API_KEY not set — Claude disabled, routing all to ATLASCore.")
        else:
            try:
                import anthropic
                self._client = anthropic.Anthropic(api_key=key)
                log.info("Claude Brain ready (%s).", self._model)
            except ImportError:
                log.error("anthropic package missing — pip install anthropic")
    ...
    def set_core(self, core) -> None:
        self._core = core
        log.info("ATLASCore wired into ClaudeBrain.")

    def set_self_improve(self, engine) -> None:
        self._self_improve = engine
        log.info("SelfImproveEngine wired into ClaudeBrain.")
    ...
    def handle(self, text: str) -> str:
        """Route text and return a voice-friendly response string."""
        text = text.strip()
        if not text:
            return ""

        # 1 — Meta commands (routing mode + session management)
        ...
        if route == "claude":
            response = self._ask_claude_with_history(text)
        else:
            response = self._core.handle(text) if self._core else self._no_core()

        # 4 — Update history and persist
        self._add_history("user", text)
        if response:
    ...
    def ask(self, text: str) -> str:
        """Single-turn query — no history update. Used by self_improve."""
        text = text.strip()
        if not text:
            return ""
        if self._client:
            return self._raw_claude([{"role": "user", "content": text}])
        return self._core.ask(text) if self._core else self._no_core()

    # ── Routing logic ─────────────────────────────────────────────────────────

    def _route(self, text: str) -> str:
        """Return 'claude' or 'core'."""
        if self._routing_mode == "claude":
            return "claude"
        ...
        # Control / system / web → always core
        if any(kw in lower for kw in _CORE_KEYWORDS):
            return "core"

        # No Claude → always core
        if not self._client:
            return "core"

        # Claude keywords → Claude
        if any(kw in lower for kw in _CLAUDE_KEYWORDS):
            return "claude"

        # Long queries (≥ 15 words) → Claude
        if len(text.split()) >= 15:
            return "claude"

        # Short conversational → core (MLX/Groq is fast)
        return "core"
    ...
    def _handle_meta(self, text: str) -> Optional[str]:
        ...
        # Force Claude
        if any(p in lower for p in ("think harder", "use claude", "deep think")):
            self._routing_mode = "claude"
            return (f"Understood, {self._user_name}. "
                    "I'll route the next response through my full reasoning engine.")

        # Force core / Groq
        if any(p in lower for p in ("use groq", "use mlx", "quick mode", "fast mode", "use fast")):
            self._routing_mode = "core"
            return f"Switching to fast mode, {self._user_name}."

        # Auto routing
        if any(p in lower for p in ("auto route", "automatic mode", "auto mode", "auto routing")):
            self._routing_mode = "auto"
            return f"Back to automatic routing, {self._user_name}."

        # Pending confirmation (self-improve apply)
        if any(p in lower for p in ("confirm", "yes apply", "go ahead", "apply it", "apply changes", "apply them")):
            if self._self_improve and self._self_improve.has_pending():
                return self._self_improve.apply_pending()

        # Summarise current session
        if any(p in lower for p in ("summarise our conversation", "summarize our conversation",
                                     "summarise the conversation", "summarize the conversation",
                                     "what have we talked about", "summarise this session",
                                     "summarize this session")):
            return self._summarize_session()

        # Load yesterday
        if any(p in lower for p in ("what did we talk about yesterday", "yesterday's conversation",
                                     "load yesterday", "what did you say yesterday")):
    ...
    def _ask_claude_with_history(self, text: str) -> str:
        """Call Claude with the current conversation history + new user turn."""
        # Build message list from history, append current query
        messages = self._sanitized_history()
        messages.append({"role": "user", "content": text})
        return self._raw_claude(messages)

    def _raw_claude(self, messages: list[dict]) -> str:
        """Low-level Claude call with explicit messages list."""
        with self._lock:
            try:
                ...
                log.error("Claude error (%s): %s", type(exc).__name__, exc)
                # Graceful fallback to core
                if self._core and messages:
                    ...
                    if last_user:
                        log.info("Claude failed — falling back to ATLASCore.")
    ...
    def _sanitized_history(self) -> list[dict]:
        """Return history with guaranteed user/assistant alternation."""
        out: list[dict] = []
        last_role = None
        for msg in self._history[-self._max_history:]:
            if msg["role"] == last_role:
                continue  # skip duplicates that would break alternation
    ...
    def _load_session(self, days_ago: int = 1) -> str:
        try:
            ...
            if not path.exists():
                return (f"I don't have a saved conversation from {target}, "
                        f"{self._user_name}.")
            ...
            # Summarise with Claude if available
            if self._client:
                summary_prompt = (
                    f"Summarise this ATLAS conversation from {target} in 3 to 5 "
                    f"sentences, written for voice output. Highlight the main topics "
                    f"discussed and any conclusions. Refer to the user as {self._user_name}.\n\n"
                    + "\n".join(
                        f"{m['role'].upper()}: {m['content']}"
                        for m in msgs[:30]
                    )
                )
                return self._raw_claude([{"role": "user", "content": summary_prompt}])
    ...
    def _summarize_session(self) -> str:
        if not self._history:
            return (f"We haven't discussed anything this session yet, "
                    f"{self._user_name}.")
        if self._client:
            prompt = (
                f"Summarise our current ATLAS session in 3 to 5 sentences for voice. "
                f"Highlight the main topics and any decisions. "
            ...
            return self._raw_claude([{"role": "user", "content": prompt}])
